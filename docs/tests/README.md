...
| Historical cached-cost backfill | SQLite initialization reprices older rows that report cached input tokens but lack cached-pricing snapshot metadata, including rows where cached usage must be recovered from stored response bodies. | `test_init_db_backfills_historical_cached_token_costs` |
