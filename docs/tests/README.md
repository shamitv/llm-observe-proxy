...
| Token usage extraction | Token counts are extracted from OpenAI-compatible `usage`, cached-token detail aliases, llama.cpp `timings`, and Ollama-style metric fields, with safe handling for partial provider metrics. | `test_extract_token_usage_supports_chat_responses_and_responses_api`, `test_extract_token_usage_prefers_openai_usage_over_provider_fallbacks`, `test_extract_token_usage_supports_provider_fallback_shapes`, `test_extract_token_usage_handles_partial_provider_metrics` |
...
| Streaming token usage extraction | Final SSE events are parsed for OpenAI-compatible `usage`, llama.cpp `timings`, and Ollama-style metric fields without decoding the whole stream when the final event is identifiable. | `test_stream_token_usage_reads_final_sse_usage_event`, `test_stream_token_usage_reads_final_sse_timings_event`, `test_stream_token_usage_reads_final_sse_ollama_metric_event` |
| Streaming cost snapshots | Streaming requests with a final OpenAI-compatible usage event or llama.cpp timings event snapshot estimated cost. | `test_streaming_request_snapshots_cost_when_usage_event_is_present`, `test_streaming_request_snapshots_cost_from_timings_event` |
