...
| Streaming compatibility rewrite | Opted-in Chat Completions streams with Qwen `<tool_call>` blocks in `reasoning_content` or `reasoning` are rewritten to `delta.tool_calls`, split SSE chunks are handled, final `finish_reason` becomes `tool_calls`, and raw upstream bytes are audited. | `test_streaming_qwen_reasoning_tool_call_is_rewritten_and_audited`, `test_streaming_qwen_reasoning_field_is_rewritten_by_default_fix` |
| Compatibility pass-through | Without configured fixes, Qwen tagged reasoning streams are preserved byte-for-byte. Malformed tagged blocks pass through unchanged with warnings and raw upstream audit storage. Existing structured streamed tool calls pass through unchanged. | `test_no_configured_fixes_preserves_qwen_reasoning_leak_byte_for_byte`, `test_malformed_qwen_reasoning_tool_call_passes_through_with_warning`, `test_streaming_transformer_preserves_existing_structured_tool_calls` |
| Non-streaming compatibility rewrite | Opted-in non-streaming Chat Completions responses with Qwen tagged reasoning are rewritten to structured `message.tool_calls` and audited. | `test_non_streaming_qwen_reasoning_tool_call_is_rewritten_and_audited` |
